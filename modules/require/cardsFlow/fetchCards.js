		...
		if(typeof cards === "undefined" || force){
			...
			try{
				...
				s.invokeOperation(
					...
					(error) => { //onFailure
						cards = [];
						kony.pubsub.publish("fetchCards.failure", error);
					},
					{} //options
				);
			}
				...
				if (errorMsg.indexOf('Invalid session') >= 0 || errorMsg.indexOf('session expired') >= 0) {
					kony.pubsub.publish("fetchCards.failure", e);
				}
				else{
					kony.pubsub.publish("fetchCards.failure", new Error(`Could not find or call ${service}.${operation}:\n\t${e}`));
				}
			}
		}
